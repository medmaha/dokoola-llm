...
MESSAGES = [
    ...
    {
        "role": "system",
        "content": "A digital marketplace connecting talent and opportunities across The Gambia and soon Africa"
    },
    {
        "role": "system",
...
def llm_text_completion(request:TextCompletionRequest, userPublicId: str, respone: Response):
    try:
        ...
        [completion, status_code] = engage_ai_for_text_completion(request.text, user)
        if not completion:
...
def engage_ai_for_text_completion(prompt: str, user: AuthUser, max_retries=3) -> str|None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Failed to get a valid response from DOKOOLA_LLM model: [{_model}]. Status code: {response.status_code}")
        ...
        if max_retries > 0:
            ...
            logger.info(f"Retrying completion with AI model [{MODELS[current_model_index+1]}]")
            return engage_ai_for_text_completion(prompt=prompt, user=user, max_retries=max_retries)
