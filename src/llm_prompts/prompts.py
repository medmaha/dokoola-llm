...
from .models import JobDescriptionPromptModel, ProposalCoverLetterPromptModel
...
def get_talent_short_bio_prompt(profile: dict) -> str:
...
def get_proposal_cover_letter_prompt(
    ...
    client_name = job.third_party_metadata.get("company_name") if job.third_party_metadata else job.client.name
    ...
    badge_text = "Pro" if badge == "pro" else "Top-Rated Star" if badge == "star" else "verified talent"
    
    job_duration_text = f"Project Duration: {job.estimated_duration}" if job.estimated_duration else ""
    job_type_text = f"Job Type: {job_type}" if job_type else ""
    job_description_text = f"Full Job Description: \"\"\"{job.description}\"\"\"" if job.description else "No job description provided."
    job_benefits_text = f"Job Benefits Offered: {', '.join(benefits)}" if benefits else ""
    resume_text = f"Relevant Experience (from resume): {resume.get('description', '')}" if resume else ""
    ...
    additional_text = f"Additional instructions from me: {additional_notes}" if additional_notes else ""
    if job.third_party_metadata and (company_name := job.third_party_metadata.get("company_name")):
...
Tone: {tone}
Length: {length}
Style: Natural, human, engaging — never robotic or generic. Avoid clichés like "I am passionate about" unless it feels authentic.
...
def get_job_description_prompt(data: DescriptionParams, user: AuthUser) -> str:
    ...
    if pricing and pricing.budget and pricing.currency and pricing.currency.symbol:
        ...
        if pricing.fixed_price:
            budget_info = f"{currency_symbol}{project_budget} fixed-price" if project_budget else "Fixed-price project"
        else:
            budget_info = f"{currency_symbol}{project_budget}/hr" if project_budget else "Hourly rate (budget negotiable)"
    else:
        budget_info = None
    ...
    if job.application_deadline:
        date_obj = datetime.fromisoformat(job.application_deadline.replace("Z", "+00:00"))
